  ...
  return (
    <section className="relative h-64 md:h-[380px] flex items-end text-white mt-16 md:mt-[72px]">
      ...
      <div className="relative z-10 w-full pb-10 md:pb-14 px-6 md:px-16 max-w-6xl mx-auto hero-reveal" style={{ animationDelay: '0.1s' }}>
        {breadcrumb.length > 0 && (
          <nav className="mb-3 text-xs text-white/55 tracking-wider uppercase flex items-center gap-2">
            <Link to="/" className="text-white/55 hover:text-white no-underline transition-colors">
              Accueil
            </Link>
            {breadcrumb.map((item, i) => (
              <span key={i} className="flex items-center gap-2">
                <span>/</span>
                {item.to ? (
                  <Link to={item.to} className="text-white/55 hover:text-white no-underline transition-colors">
                    {item.label}
                  </Link>
                ) : (
        ...
        {subtitle && (
          <p className="text-white/70 text-sm md:text-base font-light mt-3 max-w-xl">{subtitle}</p>
        )}
